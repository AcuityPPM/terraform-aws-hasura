...
resource "aws_db_instance" "hasura" {
  ...
  vpc_security_group_ids = [aws_security_group.hasura_rds.id, aws_security_group.allow_internal_postgres_in.id, var.rds_whitelist_sg, var.rds_whitelist_lambda_sg]
...
resource "aws_cloudwatch_metric_alarm" "ecs_task_high_memory" {
  alarm_name          = "ecs-task-${var.rds_db_name}-high-memory"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
